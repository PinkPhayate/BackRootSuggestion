...
public class HTTP: NSOperation {
    ...
    */
    public class func New(url: String, method: HTTPVerb, parameters: HTTPParameterProtocol? = nil, headers: [String:String]? = nil, requestSerializer: HTTPSerializeProtocol = HTTPParameterSerializer()) throws -> HTTP  {
        ...
        print("\(#function) in \(((#file as NSString).lastPathComponent as NSString).stringByDeletingPathExtension) \(#line) cookies count:\(cookieStorage.cookies?.count)")
        
        for c in cookieStorage.cookies! {
            print("\(#function) in \(((#file as NSString).lastPathComponent as NSString).stringByDeletingPathExtension) \(#line) cookie:\(c.domain)")
            cookieStorage.deleteCookie(c)
        }
        
        guard var req = NSMutableURLRequest(urlString: url) else { throw HTTPOptError.InvalidRequest }
        req = NSMutableURLRequest(URL: NSURL(string:url)!, cachePolicy:NSURLRequestCachePolicy.ReloadIgnoringLocalCacheData, timeoutInterval: 10)
